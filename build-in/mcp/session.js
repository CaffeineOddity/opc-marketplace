 ...
 */
export function generateNextRequirementId(source = 'auto_assembled', cwd) {
...
export function readSessionIndex(cwd) {
...
// Session Binding
// ============================================================
export function bindSessionToRequirement(lockId, requirementId, source, workflowName, cwd) {
...
// Session Query
// ============================================================
export function getCurrentSession(lockId, cwd) {
    const index = readSessionIndex(cwd);
    return index.sessions[lockId] || null;
}
export function getCurrentRequirementId(lockId, cwd) {
...
// Task Listing
// ============================================================
export function listAllTasks(cwd) {
    const sessionsDir = ensureOpcDir('state/sessions', cwd);
    if (!existsSync(sessionsDir))
        return [];
 ...
 */
export function findSimilarTask(projectName, projectDescription, cwd, threshold = 0.5) {
    const sessionsDir = ensureOpcDir('state/sessions', cwd);
    if (!existsSync(sessionsDir))
        return null;
...
// Clear Task
// ============================================================
export function clearCurrentTask(cwd) {
    const lockId = getCurrentLockId(cwd);
    const index = readSessionIndex(cwd);
    if (index.sessions[lockId]) {
