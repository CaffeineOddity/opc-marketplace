 ...
 */
export function generateSessionFilename(requirementId, source) {
 ...
 */
export function getProjectStatePath(requirementId, source, cwd) {
 ...
 */
export function getSessionPathByFilename(filename, cwd) {
    const sessionsDir = ensureOpcDir('state/sessions', cwd);
    return join(sessionsDir, filename);
}
/**
 ...
 */
export function findSessionByRequirementId(requirementId, cwd) {
    const sessionsDir = ensureOpcDir('state/sessions', cwd);
...
export function readProjectState(requirementId, source, cwd) {
...
export function writeProjectState(state, cwd) {
...
// Handoffs
// ============================================================
export function getHandoffPath(lockId, cwd) {
    const stateDir = ensureOpcDir('state', cwd);
    return join(stateDir, lockId, 'handoffs.json');
}
export function recordHandoff(fromAgent, toAgent, artifacts, constraints, context, lockId, cwd) {
    const handoff = {
...
export function getHandoffs(lockId, cwd) {
    const path = getHandoffPath(lockId, cwd);
    return readJsonFile(path) || [];
}
// ============================================================
// Task Groups
// ============================================================
export function createTaskGroup(state, stage, groupName, tasks, parallel, completionCondition, threshold, cwd) {
...
export function updateTask(state, taskId, status, progress, artifact, cwd) {
    let found = false;
...
export function getTaskGroups(state, stage, groupId) {
    const groups = [];
    for (const stageName of Object.keys(state.pipeline.stages)) {
