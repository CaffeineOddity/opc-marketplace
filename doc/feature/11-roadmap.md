...
| 3 | engine/ 核心：node-resolver + phase-validator | 给一组节点和前置条件，输出正确的执行计划 |
| 4 | 第一个 kit + 第一个 node：dev-kit 的 tdd-implementation 示例节点 | 手动触发一个 node，走通 knowledge-load → agent → MCP knowledge write |
| 5 | 意图识别 + task-classifier | 输入自然语言，输出正确的 tags + suggested_phases |
| 6 | Hook 体系：knowledge-load/save、phase-transition | 整个 node 执行流程无需手动干预 |
