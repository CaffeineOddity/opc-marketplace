...
> 历史名 `opc_flow_start` / `opc_intent_complete` / `opc_task_analysis_complete` / `opc_decomposition_complete` / `opc_brief_complete` 已折叠为 `opc_flow_lifecycle({action:"start"})` 与 `opc_flow_step_complete({step})` 的 discriminator 分支；`opc_knowledge_list` 已折叠为 `opc_knowledge_read({mode:"list"})`。详见 [../../01-overview/07-tool-consolidation.md](../../01-overview/07-tool-consolidation.md)。
