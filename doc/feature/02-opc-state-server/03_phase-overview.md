...
| 子文档 | 内容 |
|------|------|
| [01_nine-phases.md](phase/01_nine-phases.md) | 9 阶段总览 + `phase.md` 元信息文件 |
| [02_node-selection.md](phase/02_node-selection.md) | 节点选择策略、反思轮次表、置信度阈值表 |
| [03_scenarios.md](phase/03_scenarios.md) | Scenario 场景配方与加权机制 |

### 阶段生命周期（按执行顺序）

| 子文档 | 内容 | 涉及工具 |
|------|------|------|
| [04_phase-start.md](phase/04_phase-start.md) | `opc_phase_start` + 自省评估 4 维度 + 三种推进路径 + 反思循环 | `opc_phase_start` / `opc_flow_reflect` |
| [05_phase-confirm-execute.md](phase/05_phase-confirm-execute.md) | `opc_phase_confirm` 锁定执行 + 逐 node 执行 | `opc_phase_confirm` / `opc_node_start` |
| [06_phase-complete-reset.md](phase/06_phase-complete-reset.md) | `opc_phase_complete` + auto_advance 规则 + `opc_phase_reset` + 分层回退 L0–L3 | `opc_phase_complete` / `opc_phase_reset` |

### 扩展能力

| 子文档 | 内容 |
|------|------|
| [07_comma-command.md](phase/07_comma-command.md) | `/comma` 阶段独立运行 + dry-run / mock-inputs |
| [08_tools-and-automation.md](phase/08_tools-and-automation.md) | 节点来源 + 6 个阶段工具汇总 + 自动机制 |

---

## 快速入口

- **进入阶段**：[`opc_phase_start`](phase/04_phase-start.md#二opc_phase_start--扫描与返回) — 由 `opc_pipeline_create` / 上一 phase 的 `opc_phase_complete` 路由触发
- **锁定执行**：[`opc_phase_confirm`](phase/05_phase-confirm-execute.md#一opc_phase_confirm--锁定执行计划) — 自省置信度 ≥ 阈值后调用
- **回退**：[`opc_phase_reset`](phase/06_phase-complete-reset.md#三opc_phase_reset--阶段重置) — 快照恢复，下游级联 pending
...
- [意图分析](01_intent-analysis-overview.md) — 任务复杂度决定反思轮次和推进策略
- [管线](02_pipeline-overview.md) — 管线状态管理
- [节点](04_node-overview.md) — 节点定义与执行
