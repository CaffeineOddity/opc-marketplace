...
| 反思预算 | — | budget-guard 中等（默认 2-3 轮） | budget-guard 宽松（默认 3-5 轮） |
...
| V1-V5 fail 或 严重 objection | 进入 P2 反思（primary=M3 CoVe，secondary=M2 Reflexion），受 budget-guard 约束；budget 耗尽 → ask_user |
