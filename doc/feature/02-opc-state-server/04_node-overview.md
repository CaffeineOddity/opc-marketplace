...
| 子文档 | 内容 |
|------|------|
| [01_types-and-definition.md](node/01_types-and-definition.md) | 节点类型（控制 vs 任务）+ 完整 markdown 定义示例 |
| [02_field-spec.md](node/02_field-spec.md) | frontmatter 全部字段：基础 + Agent + 质量门 + 超时重试 + input/output |

### 选择与编排

| 子文档 | 内容 |
|------|------|
| [03_signal-matching.md](node/03_signal-matching.md) | tag 交集 + 语义匹配 + Scenario 加权 |
| [04_concurrency-and-deps.md](node/04_concurrency-and-deps.md) | 文件域隔离、依赖解析、blocked_by 双写 |

### 执行与重试

| 子文档 | 内容 | 涉及工具 |
|------|------|------|
| [05_execution-and-retry.md](node/05_execution-and-retry.md) | 完整执行流程 + 三种重试场景 + retry_count 规则 | 4 个 `opc_node_*` |
| [07_tools.md](node/07_tools.md) | 4 个节点级工具完整规范 + Agent 委派模式 | 全部 |

### 来源与引擎

| 子文档 | 内容 |
|------|------|
| [06_source-and-override.md](node/06_source-and-override.md) | 节点项目覆盖优先级 + `plugin.json` 声明式能力 |
| [08_internal-engines.md](node/08_internal-engines.md) | node-resolver / state-manager / flow-router 三个内部引擎 + 自动机制 |

---

## 快速入口

- **启动节点**：[`opc_node_start`](node/07_tools.md#opc_node_start) — 由 `opc_phase_confirm` 路由触发
- **完成节点**：[`opc_node_complete`](node/07_tools.md#opc_node_complete) — Agent 回报 evidence 后调用
- **重跑节点**：[`opc_node_retry`](node/07_tools.md#opc_node_retry) — 手动重跑，自动级联重置下游
...
- [阶段](03_phase-overview.md) — 节点选择与阶段生命周期
- [03-1 知识模型](../03-opc-knowledge-server/01_knowledge-model-overview.md) — 知识读写与版本管理
