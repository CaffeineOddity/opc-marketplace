...
完整执行流程：从 `opc_node_start` 到 `opc_node_complete` / `opc_node_fail`，覆盖三种重试场景。
...
- `opc_node_fail` 自动 retry：`retry_count++`
- `opc_node_retry` 手动调用：`reset_retry_count=true` 默认清零
- 超时触发的自动 `opc_node_retry`：`reset_retry_count=false` 保留计数

达到 `max_retries`（默认 3）后 node 标记 `failed`，需要人工 `opc_node_retry(reset_retry_count=true)` 才能继续尝试。
...
- [07_tools.md](07_tools.md) — 4 个工具完整规范
