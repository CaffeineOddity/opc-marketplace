...
> 配套字段：[03_pipeline-plan.md 三](03_pipeline-plan.md#三字段说明) sub.status 含 `paused`，sub.execution_priority、sub.paused_at、sub.inserted_at。配套工具：[09_tools.md replan 分支](09_tools.md#opc_pipeline_lifecycle-actionreplan) 与 [resume 分支](09_tools.md#opc_pipeline_lifecycle-actionresume)。
...
**核心原则**：插队是"调度"而不是"中断"。`opc_pipeline_lifecycle({action:"replan"})` 只是入队，真正的状态翻转由 `opc_node_finish` → state-manager 在 node 边界完成。
...
sequenceDiagram
    ...
    actor U as 用户
    actor C as Claude
    participant RP as opc_pipeline_lifecycle(replan)
    participant ND as node-tools
    participant SM as state-manager
    participant RS as opc_pipeline_lifecycle(resume)
    ...
    C->>RP: opc_pipeline_lifecycle({action:"replan", changes: {add_sub_pipeline: [{<br/>id: "sub-insert-1", knowledge_unit: ["logging"],<br/>execution_priority: "immediate", phase_plan: {...}}]}})
    ...
    SM->>RS: opc_pipeline_lifecycle({action:"resume", sub_pipeline_id: "sub-2"})
...
in_progress ─── 同管线插入 immediate sub ───> paused
           │       (node 边界触发)
           │
paused ──── insert sub 完成 ─── auto opc_pipeline_lifecycle(resume) ──> in_progress
           │
           ├── 用户主动 opc_pipeline_lifecycle({action:"resume"}) ─> in_progress
           │
           └── opc_pipeline_lifecycle({action:"abort"}) ──> aborted
```
...
插队 sub 与被挂起 sub 在 `opc_pipeline_lifecycle({action:"replan"})` 阶段已校验 `knowledge_unit` 不重叠，**正常路径下不会产生 knowledge 冲突**。

但用户在挂起期间可能手工编辑被挂起 sub 的 knowledge 文件（L3 用户 git 操作）。`opc_pipeline_lifecycle({action:"resume"})` 的步骤 ④ 一致性探测会发现这种漂移：
...
| 关系 | 说明 |
|------|------|
| `opc_flow_correct({action:"phase_reset"})` | 操作单个 sub 内的阶段回退；不触发 paused/resume |
| `opc_flow_correct({action:"revise"})` | 只改 accumulated 字段；不改 sub_pipelines |
| `opc_flow_correct({action:"restart", from_step})` | 回退到流程分析步骤；插队不通过此工具 |
| `opc_pipeline_lifecycle({action:"abort"})` | 级联终止所有 sub（含 paused 和 in_progress）；放弃整管线 |
| **`opc_pipeline_lifecycle({action:"replan"})` + `add_sub_pipeline(execution_priority: "immediate")`** | 唯一插队入口 |
| **`opc_pipeline_lifecycle({action:"resume"})`** | 唯一显式恢复入口；通常由 state-manager 自动触发 |

---

## 六、与 opc_flow_defer 的取舍

早期设计曾考虑独立的 `opc_flow_defer` / `opc_flow_resume` 流程级工具。最终归并到管线层 + `opc_pipeline_lifecycle({action:"replan"})`，理由：
...
- 工具总数：管线层 3（create / status / lifecycle），流程层 7；可控
...
- [09_tools.md](09_tools.md) — `opc_pipeline_lifecycle` replan 与 resume 分支完整规范
